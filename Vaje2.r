...
upanje_P2 <- mean(knots(panjer1)^2) #mora biti priblizno 585
varianca_prava <- upanje_P2-upanje_P^2

varianca_P <- sum(diff(knots(panjer1))) #Je to ze prav?
varianca_poskus <- var((knots(panjer1)))

# d <- mean(knots(panjer1)^2)
# b <- rep(mean(panjer1), 229)
# varianca_P3 <- mean((d-b)^2)
#Popravi varianco. Upanje je pravo, varianca pa narobe?
...
#Očitno je simulacija dobra za oceno upanja.
